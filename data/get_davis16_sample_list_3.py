...
file_write_obj = open('DAVIS16_samples_list_3.txt','w')
for seq in seq_names:
    ...
    # order
    for i in range(3,len(frame_path_list)):
        line = frame_path_list[i-3]+','+frame_path_list[i-2]+','+frame_path_list[i-1]\
             + ';'+frame_path_list[i]+';'+anno_path_list[i]
        file_write_obj.writelines(line)
        file_write_obj.write('\n')
    # border
    line = frame_path_list[0] + ',' + frame_path_list[0] + ',' + frame_path_list[
    ...
    file_write_obj.writelines(line)
    file_write_obj.write('\n')
    line = frame_path_list[0] + ',' + frame_path_list[1] + ',' + frame_path_list[
        2] + ';' + frame_path_list[3] + ';' + anno_path_list[3]
    file_write_obj.writelines(line)
    file_write_obj.write('\n')
    # reverse order
    for i in range(0,len(frame_path_list)-3):
        line = frame_path_list[i+3]+','+frame_path_list[i+2]+','+frame_path_list[i+1]\
             + ';'+frame_path_list[i]+';'+anno_path_list[i]
        file_write_obj.writelines(line)
        file_write_obj.write('\n')
